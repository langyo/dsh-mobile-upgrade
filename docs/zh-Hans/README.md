...
上文每个功能都可以在 设置 → 插件 → dsh-mobile-upgrade 里切换（下次页面加载生效），也可以按设备用 `localStorage` 键覆盖——`mfx-attach`、`mfx-restart`、`mfx-settle`、`mfx-drawer`、`mfx-settings`、`mfx-modality`、`mfx-menus`、`mfx-questions`——值为 `"0"` 即关闭该功能。

## 已知限制

- 客户端按宿主 UI 的哈希 CSS 类名（菜单、抽屉开关、供应商编辑器）挂钩。宿主构建若重命名这些类，对应功能会降级，待本插件跟进适配；其余功能不受影响。问题卡片中的问题区域则改由卡片自身的 `data-question-key` 钩子定位，因此宿主重建若只是重新哈希 CSS 模块，它仍能继续工作。
