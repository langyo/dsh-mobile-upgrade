...
上記の各機能は 設定 → プラグイン → dsh-mobile-upgrade で切り替えられます（次回のページ読み込みで反映）。`localStorage` キー — `mfx-attach`、`mfx-restart`、`mfx-settle`、`mfx-drawer`、`mfx-settings`、`mfx-modality`、`mfx-menus` — でデバイスごとに上書きでき、値 `"0"` で無効になります。
