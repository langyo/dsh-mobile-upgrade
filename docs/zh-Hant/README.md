...
上文每個功能都可以在 設定 → 外掛 → dsh-mobile-upgrade 裡切換（下次頁面載入生效），也可以按裝置用 `localStorage` 鍵覆寫——`mfx-attach`、`mfx-restart`、`mfx-settle`、`mfx-drawer`、`mfx-settings`、`mfx-modality`、`mfx-menus`、`mfx-questions`——值為 `"0"` 即關閉該功能。

## 已知限制

- 客戶端按宿主 UI 的雜湊 CSS 類別名稱（選單、抽屜開關、供應商編輯器）掛鉤。宿主建置若重新命名這些類別，對應功能會降級，待本外掛跟進調整；其餘功能不受影響。問題卡片中的問題區域則改由卡片自身的 `data-question-key` 鉤子定位，因此宿主重新建置若只是重新雜湊 CSS 模組，它仍能繼續運作。
